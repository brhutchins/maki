...
//! Backends (e.g. `opencode`) transform this into their own resolved catalog.
...
pub(crate) type CatalogIndex = HashMap<String, CatalogProvider>;
pub(crate) type ModelKey = (String, String);

/// Read-only external surface. Construction is crate-internal via [`from_npm`].
...
impl EndpointType {
    pub(crate) fn from_npm(npm: &str) -> Self {
...
impl CatalogProvider {
    pub(crate) fn build_auth(&self, api_key: String, api_format: EndpointType) -> ResolvedAuth {
...
pub(crate) const ZEN_CATALOG_KEY: &str = "opencode";
pub(crate) const ZEN_MAKI_SLUG: &str = "opencode-zen";

/// Map a raw catalog provider id to the API/display slug used outside the
/// catalog data model. Third-party provider ids pass through unchanged; the
/// first-party "opencode" entry is exposed as "opencode-zen".
pub(crate) fn maki_slug_for(provider_id: &str) -> &str {
...
/// A single resolved model entry, shared by all backends.
#[derive(Debug, Clone)]
...
pub(crate) async fn load_cached_async() -> Option<CatalogIndex> {
...
pub(crate) async fn save_cached_async(index: &CatalogIndex) {
...
pub(crate) fn http_client(context: &str) -> isahc::HttpClient {
...
pub(crate) async fn fetch_remote_async(client: &HttpClient) -> Result<CatalogIndex, AgentError> {
